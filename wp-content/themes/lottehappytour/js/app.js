(function($) {
    $(function() {
		$('.happy-moment-slider .jcarousel li').on('click',function(event){
			...
			h2.html(data.name+'<span>'+data.date+'</span>');
			newContent = '';
			if(!$(this).closest('.group.happy-moment-wp').hasClass('spirit')){
				...
				html = '<iframe width="100%" height="450px" src="http://www.youtube.com/embed/'+data.vid+'?autohide=1&modestbranding=1&showinfo=0" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
