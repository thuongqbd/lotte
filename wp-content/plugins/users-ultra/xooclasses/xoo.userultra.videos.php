...
//		add_action( 'wp_ajax_reload_videos', array( $this, 'reload_videos' ));			
	...
	public function ini_video_module()
	{
		global $wpdb;
			...
			// Create table
			$query = 'CREATE TABLE `' . $wpdb->prefix . 'usersultra_video_galleries` (
		...
		// Create table videos
		$query = 'CREATE TABLE `' . $wpdb->prefix . 'usersultra_videos` (
				`video_id` BIGINT(20) NOT NULL AUTO_INCREMENT,
				`video_gal_id` BIGINT(20) NOT NULL,
				`video_user_id` BIGINT(20) NOT NULL,
				`video_name` VARCHAR(200) NOT NULL,
				`video_type` VARCHAR(50) NOT NULL,
				`video_unique_vid` VARCHAR(50) NOT NULL,
				`video_image` VARCHAR(200) NOT NULL,
				`video_thumb` VARCHAR(200) DEFAULT NULL,
				`video_main` INT(1) NOT NULL,
	...
	public function add_new_video ()
	{
		...
		{			
			...
			if ( $wpdb->insert( $wpdb->prefix . 'usersultra_videos', $new_message, array( '%d', '%s','%s', '%s', '%s' , '%s', '%s' , '%s' ) ) )
			{


			}
			
			echo $this->post_media_display($gal_id,null,'new_video_div');
		} //end user loged in
	...
	public function add_new_video_gallery ()
	{
		...
		if (is_user_logged_in() && isset($user_id) && isset($gall_name)) 
		{
			...
			$new_message = array(
					...
					// insert into database
					if ( $wpdb->insert( $wpdb->prefix . 'usersultra_video_galleries', $new_message, array( '%d', '%s', '%s', '%s' ) ) )
					{
  ...
  <option value='2' ".$friends.">Only Friends</option>
	...
	public function edit_video_gallery_confirm ()
	{
		...
		if($gal_id!="")
		{
			$query = "UPDATE " . $wpdb->prefix ."usersultra_video_galleries SET `gallery_name` = '$gal_name', `gallery_desc` = '$gal_desc'  , `gallery_private` = '$gal_visibility'  WHERE  `gallery_id` = '$gal_id' AND `gallery_user_id` = '$user_id' ";		
	...
	public function edit_video_confirm ()
	{
		...
		if($video_id!="")
		{
			$query = "UPDATE " . $wpdb->prefix ."usersultra_videos SET `video_name` = '$video_name', `video_unique_vid` = '$video_unique_id'  , `video_type` = '$video_type' , `video_image` = '$video_image' , `video_thumb` = '$video_thumb' WHERE  `video_id` = '$video_id' AND `video_user_id` = '$user_id' ";		
	...
	public function edit_video ()
	{
		...
		if($video_id!="")
		{
			...
			foreach ( $res as $video )
			{
				...
				if($video->video_type=='youtube'){ $youtube = "selected='selected'";}				
				if($video->video_type=='vimeo'){$vimeo = "selected='selected'";}
...
//							});
					...
					// Progress bar ////////////////////////////////////////////
					uploader<?php echo $id?>.bind('UploadProgress', function(up, file) {
						var progressBarValue = up.total.percent;
						
						jQuery('#progressbar<?php echo $id?>').fadeIn().progressbar({
							value: progressBarValue
						});
						
						jQuery('#progressbar<?php echo $id?> .ui-progressbar-value').html('<span class="progressTooltip">' + up.total.percent + '%</span>');
					});
					
					// Close window after upload ///////////////////////////////
					uploader<?php echo $id?>.bind('UploadComplete', function() {
